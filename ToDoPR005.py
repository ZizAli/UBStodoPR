...
class Task:
    ...
    def calculate_deadline_date(self):
        ...
        return today + timedelta(days=days_until_deadline)
...
class TaskManager:
    ...
    def filter_tasks(self, time_frame: TimeFrame, category: TaskCategory = None):
        ...
        for task in self.tasks:
            ...
            if time_frame == TimeFrame.TODAY and task.is_due() and task.deadline_date.date() == now.date():
                filtered_tasks.append(task)
            elif time_frame == TimeFrame.THIS_WEEK and task.is_due() and (
                    now.date() <= task.deadline_date.date() < now.date() + timedelta(days=7)):
                filtered_tasks.append(task)
            elif time_frame == TimeFrame.THIS_MONTH and task.is_due() and (
                    now.date().month == task.deadline_date.month):
                filtered_tasks.append(task)

...
penguin_image_path = './static/pinguin_53876-57854.jpg'
...
def main():
    ...
    if 'user_name' not in st.session_state:
        st.markdown(
            ...
            <style>
            .stApp {{
                background-image: url("{penguin_image_path}");
        ...
        st.title("Welcome to My TODO List App")
        ...
        # Main TODO List Page
        st.title(f"{st.session_state.user_name}'s TODO List")
        ...
        if option == "Add Event":
            ...
            if events:
                event_names = [f"{idx}: {event.name} - {event.date} - {event.category}" for idx, event in
                               enumerate(events)]
